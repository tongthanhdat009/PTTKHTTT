...
public class thongKeTheoChiPhi extends JPanel {
    private BLLThongKeDonHang bllThongKeDonHang;

    public thongKeTheoChiPhi(ArrayList<String> maCoSo) {
        this.setBounds(0, 0, 1200, 776);
        bllThongKeDonHang = new BLLThongKeDonHang();
        ArrayList<DTOThongKe> dsThongKe = bllThongKeDonHang.thongKeChiPhi(maCoSo);
        DefaultPieDataset dataset = createDataset(dsThongKe);

        // Tạo biểu đồ tròn
        JFreeChart pieChart = ChartFactory.createPieChart(
                "Chi phí theo cơ sở", // Tiêu đề biểu đồ
    ...
    private DefaultPieDataset createDataset(ArrayList<DTOThongKe> dsThongKe) {
        ...
        for (DTOThongKe thongKe : dsThongKe) {
            ...
            dataset.setValue(tenCoSo, tongGiaTri); // Thêm tổng chi phí của từng cơ sở vào dataset
        }
        return dataset;
    }
}
